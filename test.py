...
if st.button("📊 결과 보기", key="result_btn"):
    ...
    deep_ratio = stage_counts.get(0, 0) / len(stages)
    light_ratio = stage_counts.get(1, 0) / len(stages)
    rem_ratio = stage_counts.get(2, 0) / len(stages)
    awake_ratio = stage_counts.get(3, 0) / len(stages)

    # -----------------------------
    # 맞춤 피드백 (카테고리별)
    # -----------------------------
    st.subheader("🔍 맞춤 수면 피드백")

    env_tips, habit_tips, stress_tips = [], [], []

    if deep_ratio < 0.2:
        env_tips.append("🌙 침실을 **조용하고 어둡게** 유지하세요.")
        habit_tips.append("🕒 **규칙적인 취침/기상 시간**을 지켜보세요.")
        stress_tips.append("😌 자기 전 **명상·호흡 운동**으로 긴장을 완화하세요.")

    if rem_ratio < 0.15:
        habit_tips.append("📵 취침 전 **전자기기 사용을 줄이세요.**")
        habit_tips.append("🏃 낮에 **적절한 신체 활동**을 늘려보세요.")
        stress_tips.append("🎶 스트레스 해소를 위해 **독서나 음악 감상**을 해보세요.")

    if awake_ratio > 0.1:
        habit_tips.append("☕ 오후에는 **카페인 섭취를 피하세요.**")
        habit_tips.append("🚭 알코올·흡연을 줄여보세요.")
        env_tips.append("🛏️ 필요 시 **수면 클리닉 상담**을 고려하세요.")

    if not (env_tips or habit_tips or stress_tips):
        st.success("🎉 전반적으로 균형 잡힌 수면 패턴을 보이고 있습니다. 지금의 생활 습관을 유지하세요!")
    else:
        if env_tips:
            st.markdown("#### 🌙 환경 관리")
            for tip in env_tips:
